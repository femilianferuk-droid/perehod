...
# Берем токен ТОЛЬКО из переменных окружения - просто BOT_TOKEN
BOT_TOKEN = os.environ.get("BOT_TOKEN")
if not BOT_TOKEN:
    raise ValueError(
        "❌ ОШИБКА: Не найден BOT_TOKEN в переменных окружения!\n"
        "Добавь переменную окружения BOT_TOKEN в панели управления хостингом."
    )
...
print("="*60)
print(f"✅ Бот запущен с токеном: {BOT_TOKEN[:10]}...{BOT_TOKEN[-5:]}")
...
async def create_bot_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"📌 <b>Важно:</b> Реферальный код будет ФИКСИРОВАННЫМ: <code>{FIXED_REFERRAL_CODE}</code>\n\n"
        "Где взять токен?\n"
...
async def process_token(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Примеры: Casino King, Golden Casino, Lucky Bot\n\n"
